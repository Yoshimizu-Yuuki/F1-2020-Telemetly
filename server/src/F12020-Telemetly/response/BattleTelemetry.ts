...
export function createBattleTelemetryResponse(
  ...
  const playerIndex = carTelemetryData
    ? parseInt(carTelemetryData.m_header.m_playerCarIndex.toString(), 10)
    : 0;
  const playerCarPosition = lapData
    ? parseInt(lapData.m_lapData[playerIndex].m_carPosition.toString(), 10)
    : 0;
  const data: BattleTelemetryResponse[] = [
    ...
    .map((el, index) => {
      ...
      return {
        ...
        tyresDamage: carStatusData
          ? carStatusData.m_carStatusData[index].m_tyresDamage.map((el) => {
              return parseInt(el.toString(), 10);
            })
          : undefined,
        tyresAgeLaps: carStatusData
          ? parseInt(
              carStatusData.m_carStatusData[index].m_tyresAgeLaps.toString(),
              10
            )
          : undefined,
        ...
        drsActivationDistance: carStatusData
          ? parseFloat(
              carStatusData.m_carStatusData[
                index
              ].m_drsActivationDistance.toString()
            )
          : undefined,
        drs: carTelemetryData
          ? parseInt(
              carTelemetryData.m_carTelemetryData[index].m_drs.toString(),
              10
            )
          : undefined,
        speed: carTelemetryData
          ? parseInt(
              carTelemetryData.m_carTelemetryData[index].m_speed.toString(),
              10
            )
          : undefined,
        gear: carTelemetryData
          ? parseInt(
              carTelemetryData.m_carTelemetryData[index].m_gear.toString(),
              10
            )
          : undefined,
        throttle: carTelemetryData
          ? parseFloat(
              carTelemetryData.m_carTelemetryData[index].m_throttle.toString()
            )
          : undefined,
        brake: carTelemetryData
          ? parseFloat(
              carTelemetryData.m_carTelemetryData[index].m_brake.toString()
            )
          : undefined,
        ersStoreEnergy: carStatusData
          ? parseFloat(
              carStatusData.m_carStatusData[index].m_ersStoreEnergy.toString()
            )
          : undefined,
    ...
    .sort(
      (
        firstEl: BattleTelemetryResponse,
        secondEl: BattleTelemetryResponse
      ): number => {
